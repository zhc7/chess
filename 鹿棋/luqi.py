    ...
    def get(self):
        while True:
            try:
                ...
                break
        return ''.join(filter(lambda x: x in '0zZ', r.content.decode(
            "utf-8")))  # 解码后得到example：'\ufeff\ufeff0000000z0000Z000ZZZ0Z0Z0Z<br>'，通过筛选去除不需要的字符
...
def transfer_to_board_size(reading_size_board, config):
    ...
    filled_board = ''
    board_x, board_y = eval(config['board_size'])
...
def get_the_board(code):
    """获取列表0Zz形式的现有棋盘"""
    board_obj = ChessBoard(code, *tpe(code))
    board = board_obj.get()
    cut_board = []
    for i in range(5):
...
def set_new_board(code, board):
    ...
    return board_obj.set(str_board)  # 重设网上棋盘
...
if __name__ == '__main__':
    ...
    try:
        main(input_code, input_chess_type)
    except Exception:
        logging.debug(traceback.format_exc())
        raise
